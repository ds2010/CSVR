...
def beta_val(x, y):
    ...
    # CSVR
    alpha, beta, ksia, ksib = CSVR.CSVR(y, x, epsilon=0.01, u=5)
    ...
    # LCR
    # alpha, beta, epsilon = LCR.LCR(y, x, L=1)
    ...
    val = np.concatenate((csvr_val.T, cnls_val.T), axis=1)
...
y = np.array(y)
cof = beta_val(x,y)
df = pd.DataFrame(cof, index=['NOX', 'DIS', 'PTRATIO', 'INDUS', 'TAX', 'ZN', 'RAD', 'alpha'], 
