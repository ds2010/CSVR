...
data = np.vstack((svr_mse(x, y, i_mix), svrp_mse(x, y, i_mix))).T
df = pd.DataFrame(data, columns = ['svr', 'std', 'svrp', 'std'], index=['mse_out', 'mse_in', 'mape_out', 'mape_in', 'mae_out', 'mae_in', 'me_out', 'me_in'])
df.to_excel('measure_svr.xlsx')